...
-- Cost annotation rewriting was removed: even explicit calls preserve source.
run_test("cost annotation compatibility call leaves source unchanged", function()
    local before = vim.api.nvim_buf_get_lines(0, 0, -1, false)
    test_assert(not autofill.enhance_cost_basis(), "Cost rewriting is disabled")
    test_assert(vim.deep_equal(before, vim.api.nvim_buf_get_lines(0, 0, -1, false)), "Preserve source")
end)
